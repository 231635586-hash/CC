...
/**
 * 物流公司 我的 Tab（v0.3-MVP）
 *
 * 内容：当前公司信息 + 切换角色 + 设置/帮助/关于
 * 依赖：roleStore (currentRole) + MOCK_LOGISTICS_COMPANIES
 */
...
const co = MOCK_LOGISTICS_COMPANIES[0]
  ...
  <view class="tab-pane">
    <view class="me-profile">
      <view class="me-avatar">
        <text>{{ co.name.charAt(0) }}</text>
      </view>
      <view class="me-info">
        <text class="me-name">{{ co.name }}</text>
        <text class="me-phone">{{ co.phone }}</text>
        <text class="me-role">当前角色：物流公司</text>
      </view>
    </view>
    ...
    <view class="me-list">
      ...
      <view class="me-item" @click="uni.showToast({ title: '关于 - v0.3.0-MVP', icon: 'none' })">
        <view class="me-item-icon">
