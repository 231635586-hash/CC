...
import { mockYards } from '@/mock/seed'
import { FALLBACK_SALES_USER } from '@/mock/constants'
...
export const useInventoryStore = create<InventoryState>((set, get) => ({
  ...
  loadList: async () => {
    ...
    // 兜底：
    // 1) 历史库存（无业务员字段）回填为 FALLBACK_SALES_USER
    // 2) yardName 由 mockYards 统一注入，避免 seed 数据冗余写死园区名
    const yardMap = new Map(mockYards.map((y) => [y.id, y.name]))
    const safe = mockInventory.map((i) => {
      const base = i.salesPersonName
        ? i
        : { ...i, salesPersonId: FALLBACK_SALES_USER.id, salesPersonName: FALLBACK_SALES_USER.name }
      return {
        ...base,
        yardName: i.yardId ? yardMap.get(i.yardId) ?? i.yardName : i.yardName,
      }
    })
