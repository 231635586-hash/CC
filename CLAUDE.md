...
**用户提需求时必须附替代方案，AI 必须主动质疑决策。** 禁止"已想清楚"式的盲执行。

### 用户侧规则
提交需求时**必须**附带"已考虑过的替代方案"（至少 1 个），格式：
...
### 反例场景（2026-07-08 复盘）
- "加 4 列到车辆位置表" → AI 默默加上 → 实际上用户根本不会看这 4 列
- "做一个复杂的下拉筛选" → AI 默默做完 → 实际上 1 个搜索框就够用

### Why
2026-07-08 致命漏洞复盘识别：产品经理最大的价值是决策，但**没有反对意见的决策是危险的**。AI 不该只是执行器。

### 关联记忆
- [decision-challenge-mechanism.md](.claude/projects/-Users-roro-Vibe-coding/memory/decision-challenge-mechanism.md)

---

*本节最后更新: 2026-07-08 — 来源: 致命漏洞修复执行清单 P3-8*
